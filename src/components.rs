use leptos::prelude::*;

use crate::images::{Theme, Translate};
use crate::projects::Project;
use crate::utils::t;

#[component]
pub fn ThemeSwitcher(theme: ReadSignal<bool>, set_theme: WriteSignal<bool>) -> impl IntoView {
    view! {
        <button class="button__theme" on:click=move |_| {
            set_theme.set(!theme.get())
        }>
            <Theme theme />
        </button>
    }
}

#[component]
pub fn LanguagePicker(theme: ReadSignal<bool>, set_lang: WriteSignal<String>) -> impl IntoView {
    let (open, set_open) = signal(false);
    let languages = vec![("en", "🇺🇸 English"), ("es", "🇲🇽 Español")];

    view! {
        <div class="language__picker">
            <button class="button__theme language__dropdown" on:click=move |_| set_open.set(!open.get())>
                <Translate theme />
            </button>

            <Show
                when=move || open.get()
                fallback=|| view! { <div></div> }
            >
                <ul class="language__menu">
                    {languages.iter()
                        .map(|(code, label)| {
                            let code = code.to_string();
                            let label = label.to_string();
                            view! {
                                <li
                                    class="dropdown__item"
                                    on:click=move |_| {
                                        set_lang.set(code.to_string());
                                        set_open.set(false);
                                    }
                                >
                                    {label}
                                </li>
                            }
                        })
                        .collect::<Vec<_>>()}
                </ul>
            </Show>
        </div>
    }
}

#[component]
fn AvatarIcon() -> impl IntoView {
    view! {
    <div class="avatar">
            <svg class="progress-ring" width="84" height="84">
            <circle
                class="progress-ring-circle"
                stroke="currentColor"
                stroke-width="2"
                fill="transparent"
                r="40"
                cx="42"
                cy="42"
            />
        </svg>
        <img class="default" src="public/img-user.jpg"/>
        <img class="hover" src="public/img-pedri.jpg"/>
    </div>
    }
}

// Shouldn't go here but meh
#[component]
pub fn Profile(theme: ReadSignal<bool>) -> impl IntoView {
    view! {
        <div class="profile__container">
            <div class="user__container">
                <AvatarIcon />
                <div class="user__info">
                    <h3 class="kodchasan-bold">"Victor Rodriguez"</h3>
                    <div class="location__info">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke-width="1.5"
                            stroke={move || if theme.get() { "#cef1eb" } else { "#0b3e38" }}
                            class="size-6"
                        >
                          <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                          <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" />
                        </svg>
                        <p class="kodchasan-regular">"Guadalajara, Jalisco"</p>
                    </div>
                </div>
            </div>

            <div class="social__container">
                <a href="mailto:awtGerry@gmail.com" target="_blank">
                    <svg  xmlns="http://www.w3.org/2000/svg"  width="15"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-at"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 12m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M16 12v1.5a2.5 2.5 0 0 0 5 0v-1.5a9 9 0 1 0 -5.5 8.28" /></svg>
                </a>
                <a href="https://github.com/awtGerry" target="_blank">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="72 72 496 496">
                        <path
                            d="M237.9 461.4C237.9 463.4 235.6 465 232.7 465C229.4 465.3 227.1 463.7 227.1 461.4C227.1 459.4 229.4 457.8 232.3 457.8C235.3 457.5 237.9 459.1 237.9 461.4zM206.8 456.9C206.1 458.9 208.1 461.2 211.1 461.8C213.7 462.8 216.7 461.8 217.3 459.8C217.9 457.8 216 455.5 213 454.6C210.4 453.9 207.5 454.9 206.8 456.9zM251 455.2C248.1 455.9 246.1 457.8 246.4 460.1C246.7 462.1 249.3 463.4 252.3 462.7C255.2 462 257.2 460.1 256.9 458.1C256.6 456.2 253.9 454.9 251 455.2zM316.8 72C178.1 72 72 177.3 72 316C72 426.9 141.8 521.8 241.5 555.2C254.3 557.5 258.8 549.6 258.8 543.1C258.8 536.9 258.5 502.7 258.5 481.7C258.5 481.7 188.5 496.7 173.8 451.9C173.8 451.9 162.4 422.8 146 415.3C146 415.3 123.1 399.6 147.6 399.9C147.6 399.9 172.5 401.9 186.2 425.7C208.1 464.3 244.8 453.2 259.1 446.6C261.4 430.6 267.9 419.5 275.1 412.9C219.2 406.7 162.8 398.6 162.8 302.4C162.8 274.9 170.4 261.1 186.4 243.5C183.8 237 175.3 210.2 189 175.6C209.9 169.1 258 202.6 258 202.6C278 197 299.5 194.1 320.8 194.1C342.1 194.1 363.6 197 383.6 202.6C383.6 202.6 431.7 169 452.6 175.6C466.3 210.3 457.8 237 455.2 243.5C471.2 261.2 481 275 481 302.4C481 398.9 422.1 406.6 366.2 412.9C375.4 420.8 383.2 435.8 383.2 459.3C383.2 493 382.9 534.7 382.9 542.9C382.9 549.4 387.5 557.3 400.2 555C500.2 521.8 568 426.9 568 316C568 177.3 455.5 72 316.8 72zM169.2 416.9C167.9 417.9 168.2 420.2 169.9 422.1C171.5 423.7 173.8 424.4 175.1 423.1C176.4 422.1 176.1 419.8 174.4 417.9C172.8 416.3 170.5 415.6 169.2 416.9zM158.4 408.8C157.7 410.1 158.7 411.7 160.7 412.7C162.3 413.7 164.3 413.4 165 412C165.7 410.7 164.7 409.1 162.7 408.1C160.7 407.5 159.1 407.8 158.4 408.8zM190.8 444.4C189.2 445.7 189.8 448.7 192.1 450.6C194.4 452.9 197.3 453.2 198.6 451.6C199.9 450.3 199.3 447.3 197.3 445.4C195.1 443.1 192.1 442.8 190.8 444.4zM179.4 429.7C177.8 430.7 177.8 433.3 179.4 435.6C181 437.9 183.7 438.9 185 437.9C186.6 436.6 186.6 434 185 431.7C183.6 429.4 181 428.4 179.4 429.7z"
                            fill={move || if theme.get() { "#fff" } else { "#24292f" }}
                        />
                    </svg>
                </a>
                <a href="https://www.linkedin.com/in/awtgerry" target="_blank">
                    <svg viewBox="1 2 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M28.5 0.749993H2.5C1.91008 0.74399 1.34187 0.972255 0.920047 1.3847C0.498227 1.79715 0.257253 2.36009 0.25 2.94999V29.055C0.258564 29.644 0.500115 30.2057 0.921793 30.6171C1.34347 31.0284 1.91093 31.256 2.5 31.25H28.5C29.09 31.2547 29.6578 31.0256 30.0794 30.6129C30.501 30.2002 30.7421 29.6374 30.75 29.0475V2.94249C30.7395 2.35432 30.4972 1.79406 30.0759 1.38351C29.6546 0.972955 29.0883 0.745288 28.5 0.749993Z" fill="#0076B2"/>
                        <path d="M8.73072 13.6522H11.6323V23H8.73072V13.6522ZM10.1823 9C10.5151 9 10.8404 9.09883 11.1171 9.28399C11.3938 9.46915 11.6094 9.73231 11.7367 10.0402C11.864 10.3481 11.8972 10.6868 11.8322 11.0136C11.7671 11.3404 11.6067 11.6406 11.3713 11.8761C11.1359 12.1116 10.836 12.2719 10.5095 12.3367C10.1831 12.4015 9.84478 12.3679 9.53741 12.2402C9.23003 12.1124 8.9674 11.8963 8.78274 11.6191C8.59808 11.3419 8.49968 11.016 8.5 10.6828C8.50043 10.2364 8.67786 9.80833 8.9933 9.49279C9.30875 9.17725 9.73641 9 10.1823 9ZM13.4524 13.6522H16.2338V14.9356H16.2723C16.66 14.2009 17.6053 13.426 19.0168 13.426C21.9553 13.4196 22.5 15.3559 22.5 17.8665V23H19.5984V18.452C19.5984 17.3692 19.5792 15.9751 18.0908 15.9751C16.6023 15.9751 16.3492 17.1558 16.3492 18.3815V23H13.4524V13.6522Z" fill="#EBEFF1"/>
                    </svg>

                </a>
                <a href="public/docs/cv.pdf" target="_blank" class="resume__button">"Resume"</a>
            </div>
        </div> // profile__container
    }
}

#[component]
pub fn ShowcaseProject(project: Project, lang: ReadSignal<String>) -> impl IntoView {
    view! {
        <a href=project.url target="_blank" class="project__item__container">
            <div
                class="project__item__background"
                style:background=move || format!("radial-gradient(circle at top right, {}, color-mix(in srgb, {} 30%, black))", project.color, project.color)
            >
              <span class="ripple ripple-1"></span>
              <span class="ripple ripple-2"></span>
              <span class="ripple ripple-3"></span>
                <div class="project__item__header">
                    <h2 class="kodchasan-bold">{project.name.clone()}</h2>
                    <div class="project__item__langs">
                        {project.languages.iter().map(|lang| {
                            view! {
                                <div class="project__lang__item"
                                    style:background=lang.color().to_string()
                                    title=lang.name().to_string()
                                >
                                    <img src=lang.svg_path().to_string() />
                                </div>
                            }
                        }).collect::<Vec<_>>()}
                    </div>
                </div>
                <p class=".geist-regular project__item__description">
                    {move || t(&project.description_key, &lang.get())}
                </p>
                <div class="project__item__image">
                    <img src=project.image.to_string()/>
                </div>
            </div>
        </a>
    }
}
